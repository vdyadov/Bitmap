...
***
```int bitmap__copy_create(struct Bitmap * p_new, struct Bitmap const * p_old)```

Создать новую битмапу по первому аргументу и скопировать в нее массив из второго


* ```struct Bitmap * p_new``` - указатель на пустой битмап, в который будет скопирован *p_old
* ```struct Bitmap const * p_old``` - указатель на объект Bitmap, из которого будут копироваться биты. Должен указывать на валидный битмап.
