...
int main(void)
{
    ...
    ok(-1 == bitmap__nand(NULL, NULL), "acessing a NULL fails with -1");

    ok(-2 == bitmap__nand(&bmap, &bmap_2), 
                             "fails with wrong magic");
    
    bmap.magic = BITMAP_MAGIC;
    ok(-3 == bitmap__nand(&bmap, &bmap_2), "acessing bitmap with bit_array == NULL"
                                        " fails with -3");

    bmap.bit_array = calloc(1, sizeof(unsigned long));
    bmap.size = sizeof(unsigned long) * 8;
    
    ok(-4 == bitmap__nand(&bmap, NULL), "acessing a NULL fails with -4");

    ok(-5 == bitmap__nand(&bmap, &bmap_2), 
                             "magic number uncorrect fails with -2");

    bmap_2.magic = BITMAP_MAGIC;
    ok(-6 == bitmap__nand(&bmap, &bmap_2), "acessing bitmap with bit_array == NULL"
                                        " fails with -6");
    ...
    // 10110111... is number after nand
    check_number = 0xffffffffffffffff;
    check_number &= ~((unsigned long)1 <<(sizeof(unsigned long) * 8 - 1 - 1));
    check_number &= ~((unsigned long)1 <<(sizeof(unsigned long) * 8 - 1 - 4));
    bitmap__nand(&bmap, &bmap_2);
    ok(check_number == *bmap.bit_array, "bitwise nand work correctly");
